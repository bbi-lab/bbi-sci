...
if __name__ == '__main__':
    ...
    for r in inp_handle:
        ...
        polya = line2[11:15]
        for i in range(args.hash_edit_distance + 1):
            if hashbc in hash_whitelist[i] and polya == "AAAA":
        ...
        if cell_barc in hashdict[hashval]:
            if not umi in hashdict[hashval][cell_barc]: # bge
              num_umi_new[i] += 1 # bge
            ...
            num_umi_new[i] += 1 # bge
            hashdict[hashval][cell_barc] = set()
            hashdict[hashval][cell_barc].add(umi)
            hashcounts[i] += 1
    ...
    print("num_hash: %d" % (num_hash))
    print("num_umi_new: ", num_umi_new)
