...
process trim_fastqs {
    ...
    when:
	!params.run || ((input_fastq.name - ~/-L00\d.fastq/) in params.run) || ((input_fastq.name  - ~/-L00\d.fastq/) in params.run.collect{"$it".replaceAll(/\s/, ".").replaceAll(/_/, ".").replaceAll(/-/, ".").replaceAll(/\\)/, ".").replaceAll(/\\(/, ".").replaceAll(/\\//, ".")}))    
    """
    mkdir trim_out
