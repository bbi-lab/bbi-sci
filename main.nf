...
process trim_fastqs {
    ...
    when:
	!((input_fastq.name - ~/-L00\d.gz.fastq.gz/) in "Undetermined") && (!params.samples || ((input_fastq.name - ~/-L00\d.gz.fastq.gz/) in params.samples) || ((input_fastq.name  - ~/-L00\d.gz.fastq.gz/) in params.samples.collect{"$it".replaceAll(/\s/, ".").replaceAll(/_/, ".").replaceAll(/-/, ".").replaceAll(/\\//, ".")})) 
    """
    mkdir trim_out
