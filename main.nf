...
process trim_fastqs {
    ...
    println (input_fastq.name.replaceAll(/\s/, ".") - ~/-L00\d.fastq/)
    println params.run.collect{"$it".replaceAll(/\s/, ".")}
    println (input_fastq.name  - ~/-L00\d.fastq/) in params.run.collect{"$it".replaceAll(/\s/, ".")} 
    """
    mkdir trim_out
